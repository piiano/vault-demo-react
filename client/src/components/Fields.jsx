      ...
      <div className={className}>
        ...
        <Placeholder className="h-8"  isLoading={isLoading}>
          <div className="relative mt-1 rounded-md shadow-sm">
            ...
            <input id={id} type={type} {...props} className={clsx(inputClasses, inputClassName, 'pl-9', error ? errorClassName : '')} />
          </div>
        </Placeholder>
      </div>
...
export function SelectField({ 
  ...
  return (
    <div className={className}>
      ...
      <select id={id} {...props} className={clsx(inputClasses, inputClassName, 'pr-8', error ? errorClassName : '')} />
      { error && <div className="mt-2 text-red-600 text-xs">{error}</div> }
      { hint && <div className="mt-2 text-sm text-gray-500">{hint}</div> }
    </div>
  )
